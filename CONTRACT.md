...
**`VISION.md` (proposal v0.7, repo-canonical) is the *why*** — the stratified
...
- **House style:** tabs, double quotes, semicolons.
...
`src/lib/grammar/types.ts` is `@generated by
...
The third job is also concrete: the generated mask manifest names one integrity-stamped schema
per shipped dialect, the npm and Python distributions carry byte-identical copies, and the
Pydantic-AI lab injects the exact installed mask into each structured-output run. This proves the
package circuit; it does not claim a production slow-loop host deployment.
