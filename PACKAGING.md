...
- `compounds/` — generated promoted catalog, compound factory, registry, resolver, and gate.
- `dialects/` — generated structural policies, dialect data, active store, arrival, and provider.
...
- `py/morphe_grammar/` — Pydantic grammar/catalog authority, schema emission, TS codegen, and masks.
...
- `@rationallyprime/morphe/schemas/*` — the generated JSON Schema artifacts, mapped onto
  the repo's `schema/` tree: `schemas/morphe-grammar.schema.json` (the `Node` union),
  `schemas/morphe-decision.schema.json`, `schemas/morphe-delta.schema.json`,
  `schemas/masks/*` (the manifest, full masks, and one genuine `G|D` mask per dialect), and
  `schemas/cms/*`. Consumers can
  validate trees against the exact grammar version and generated grammar fingerprint
  they installed instead of vendoring schema files. Source of truth is
  `py/morphe_grammar/models.py`; the artifacts are regenerated by
  `python -m morphe_grammar.artifacts --write` and CI-pinned. Contract identity is
  the pair `(GRAMMAR_VERSION, GRAMMAR_FINGERPRINT)` — two decoder aggregates must
...
loads all nine masks from an isolated wheel import.
