...
- `@rationallyprime/morphe/schemas/*` — the generated JSON Schema artifacts, mapped onto
  ...
  validate trees against the exact grammar version they installed instead of vendoring
  schema files. Source of truth is `py/morphe_grammar/models.py`; the artifacts are
  regenerated by `python -m morphe_grammar.artifacts --write` and CI-pinned.
